...
const VerifyEmail = () => {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      const res = await verifyEmail(data.email, data.code);
      toast.success(res.message || 'Email verified successfully! Welcome.');
      navigate('/');
    } catch (err) {
      ...
      >
        <div className="flex flex-col items-center">
          ...
          <p className="text-xs text-white/70 mt-2 font-medium">
            We sent a 6-digit OTP code to verify your account.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5 text-left">
          ...
          <div className="flex flex-col gap-1 text-center">
            ...
            <input
              ...
              className="w-full px-4 py-3 text-center text-lg tracking-[0.5em] font-mono bg-white/10 border border-white/15 focus:border-white focus:ring-1 focus:ring-white/20 rounded-2xl outline-none text-white transition-all duration-200 font-black"
            />
            {errors.code && (
              <p className="text-[10px] text-red-400 font-bold mt-1 text-center">{errors.code.message}</p>
            )}
          </div>
        ...
        <div className="p-4 bg-white/10 border border-white/10 rounded-2xl text-center">
          <p className="text-[10px] text-white/80 leading-normal font-medium">
            💡 <strong>Testing Tip:</strong> The code is printed to the terminal console where the server is running. Check your logs!
          </p>
        </div>
      </motion.div>
    </div>
